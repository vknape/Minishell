...
/*   Updated: 2024/06/27 09:22:41 by adakheel      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
char	*remove_whitespace_at_end(char *str)
{
	int		i;
	int		j;
	char	*des;
	...
	if (str && str[j] == 34)
		i = 1;
	j -= i;
	while (str && str[j] && is_white_space(str[j]))
		j--;
	end = ft_substr(str, 0, j + 1);
	if (i)
		end = ft_strjoin_free(end, "\"");
	free(str);
	if (!end)
		return (NULL);
	return (end);
...
char	*search_dollar_signe(t_all *all, char *str)
{
	...
	free(str);
	if (found == 0)
		whole = remove_whitespace_at_end(whole);
	return (whole);
}
