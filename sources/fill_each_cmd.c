...
/*                                                   +#+                      */
/*   Created: 2024/06/27 07:27:34 by adakheel      #+#    #+#                 */
/*   Updated: 2024/06/27 07:56:32 by adakheel      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
char	**join_arrays2(char **array1, char **array2, char **temp)
{
	int	i;
	int	j;
...
char	**join_arrays(t_all *all, char **array1, char **array2)
{
	...
	return (join_arrays2(array1, array2, temp));
	//i = 0;
	// while (array1 && array1[i])
	// {
	// 	temp[i] = array1[i];
...
char	*add_file(t_all *all, t_cmd *each_cmd, t_chunk *chunks)
{
	...
	if (chunks->next->str[i])
		temp = ft_split_quotes(all, &chunks->next->str[i], ' ');
	if (!temp && errno == 12)
		memory_allocation_failed(all);
	...
	t_chunk *chunks, char *str)
{
	...
	{
		each_cmd->cmd = ft_split_quotes(all, chunks->str, ' ');
